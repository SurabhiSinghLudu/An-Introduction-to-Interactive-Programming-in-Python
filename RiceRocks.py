...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            image_index = (self.age % 25) // 1
            self.image_center = [self.image_center[0] + image_index * self.image_size[0], self.image_center[1]]
        canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
...
def group_collide(group, other_object):
    ...
    for item in set(group):
        if item.collide(other_object):
            explosion_group.add(Sprite(item.get_position(), (0, 0), 0, 0, explosion_image, explosion_info))
            group.remove(item)
            collide_count += 1
...
def draw(canvas):
    ...
    canvas.draw_text(str(lives), [110, 50], 22, "White")
    canvas.draw_text(str(score), [740, 50], 22, "White")
...
def rock_spawner():
    ...
    rock_vel[0] = rock_vel[0] * (1 + score / 3)
    rock_vel[1] = rock_vel[1] * (1 + score / 3)
    rock_avel = random.random() * .2 - .1
    if len(rock_group) <= 12:
        if dist(rock_pos, my_ship.get_position()) > 30:
